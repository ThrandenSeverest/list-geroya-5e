...
for (let index = 0; index < lines.length; index += 1) {
  ...
  // Pandoc keeps the labels bold when a component has no parenthetical detail,
  // but Word may flatten the whole metadata row when the material is specified.
  // Match both forms and stop at the next field label instead of relying on `**`.
  const field = (label, nextLabel) => text.match(new RegExp(`(?:\\*\\*)?${label}:(?:\\*\\*)?\\s*(.+?)(?=\\s+(?:\\*\\*)?${nextLabel}:|$)`, "m"))?.[1]?.trim();
  const castingTime = field("Накладывание", "Дистанция");
  const range = field("Дистанция", "Компоненты");
  const components = field("Компоненты", "Длительность");
...
const generated = records.map(record => Object.fromEntries(Object.entries(record).filter(([key]) => key !== "englishName")));
