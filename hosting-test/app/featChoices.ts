...
const strixhavenColleges = {
  "Лорхолд": { cantrips: ["light", "sacred-flame", "thaumaturgy"], classes: ["cleric", "wizard"] },
  "Призмари": { cantrips: ["firebolt", "prestidigitation", "ray-of-frost"], classes: ["bard", "sorcerer"] },
  "Квандрикс": { cantrips: ["druidcraft", "guidance", "mage-hand"], classes: ["druid", "wizard"] },
  "Сильверквилл": { cantrips: ["sacred-flame", "thaumaturgy", "vicious"], classes: ["bard", "cleric"] },
  "Визерблум": { cantrips: ["chill-touch", "druidcraft", "spare-the-dying"], classes: ["druid", "wizard"] },
} as const;
...
export function featChoiceGroups(choiceValue: AdvancementChoice, spells: CatalogSpell[], characterLevel = choiceValue.level): FeatChoiceGroup[] {
  ...
  if (feat === "strixhaven-initiate") {
    const college = choice(choiceValue, "college")[0] as keyof typeof strixhavenColleges | undefined;
    groups.push({ key: "college", title: "Факультет Стриксхейвена", description: "Предыстория или черта определяет факультет и его список заклинаний.", count: 1, options: Object.keys(strixhavenColleges).map(name => ({ id: name, name })) });
    ability(["int", "wis", "cha"]);
    if (college && strixhavenColleges[college]) {
      const rule = strixhavenColleges[college];
      groups.push({ key: "cantrips", title: "Два заговора факультета", description: "Выберите два из трёх заговоров выбранного факультета.", count: 2, options: rule.cantrips.map(id => {
        const spell = spells.find(item => item.id === id);
        return { id, name: spell?.name || id, detail: spell?.description };
      }) });
      groups.push({ key: "spell", title: "Заклинание 1-го круга", description: "Выберите одно заклинание из списка факультета. Оно доступно раз за продолжительный отдых без ячейки.", count: 1, options: spells.filter(spell => spell.level === 1 && rule.classes.some(classId => spell.classes.includes(classId))).map(spell => ({ id: spell.id, name: spell.name, detail: spell.description })) });
    }
  }
