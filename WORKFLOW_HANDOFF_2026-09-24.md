...
5. Дальнейшая работа: проверка на реальных сохранениях и PDF-рендере, расширение проверенной модели атак и маркеров, подтверждение схемы Helpmate для двух обычных заклинателей, версия UI для записи новых HP бросков; production обновлять отдельно по `HOSTING_UPDATE.md`.
