...
def create_session(db: Session, user_id: str, scope: str = "full") -> str:
    token = random_token(); db.add(AuthSession(id=str(uuid.uuid4()), user_id=user_id, token_hash=hash_token(token), expires_at=int(time.time()) + settings.session_days * 86400, created_at=now_iso(), scope=scope)); db.commit(); return token
