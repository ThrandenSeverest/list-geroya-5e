...
export function setHitPointRoll(character: ExportCharacter, characterLevel: number, roll: number | null): ExportCharacter {
  ...
  return character.currentHitPoints === undefined ? next
    : { ...next, currentHitPoints: Math.min(character.currentHitPoints, estimatedHitPoints(next)) };
}
