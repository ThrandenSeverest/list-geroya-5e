...
const siteChangelog = [{
  ...
  changes: [
    "Добавлено главное меню с быстрым переходом к созданию персонажа, сохранённым персонажам и квизу; из рабочих разделов можно вернуться в главное меню.",
    "В главное меню добавлен блок «Отдельное спасибо» с благодарностью Вельмире (@WiseHomeAI_bot) и сообществу «Круговорот Миров».",
...
function Builder() {
  ...
  const tashaPreviewSections = multiclassEntries.map(entry => ({
    key: entry.classId,
    title: classes.find(option => option.id === entry.classId)?.name || entry.classId,
    features: (sheetOptionalFeatures[entry.classId] || []).filter(feature => (feature.level || 1) <= entry.level),
            ...
            <div className="level-layout">
              ...
              {choiceGroups.length > 0 && (
                <div className="class-choice-section">
                  <div className="ability-editor-head"><div><small>Обязательные решения класса</small><h2>Настройка способностей</h2></div><span>{choiceGroups.filter(group => selectedClassChoiceIds(rulesCharacter, group).length === group.count).length} / {choiceGroups.length}</span></div>
                  ...
                  {choiceGroups.map(group => {
                    const selected = selectedClassChoiceIds(rulesCharacter, group);
                    return (
                      <section className="class-choice-group" key={group.key} data-incomplete={selected.length !== group.count}>
                        ...
                        <div className="class-choice-grid">
                          {group.options.map(option => (
                            <article key={option.id} className={`class-choice-card${selected.includes(option.id) ? " selected" : ""}${option.disabledReason ? " disabled" : ""}`}>
                              <button disabled={!!option.disabledReason} onClick={() => toggleClassChoice(group.key, option.id, group.count)}>
                                <span>{option.source}</span><strong>{option.name}</strong><p>{option.summary || option.description}</p>{option.replacesFeatureName && <small>Заменяет: {option.replacesFeatureName}</small>}{option.disabledReason && <small>Недоступно: {option.disabledReason}</small>}
                              </button>
                              {option.fullDescription && <details><summary>Подробнее</summary><p>{option.fullDescription}</p></details>}
                            </article>
                          ))}
                        </div>
                      </section>
              ...
              <div className="tasha-panel">
                ...
                {rulesCharacter.useTasha && <div className="feature-preview">{tashaPreviewSections.map(section => <section key={section.key}><h3>{section.title}</h3>{section.features.map(feature => <div key={`${feature.level}-${feature.name}`}><strong>{feature.level} ур. · {feature.name}</strong><p>{feature.description}</p></div>)}</section>)}</div>}
              </div>
              {featSlots > 0 && (
            ...
            <div className="final-workspace">
              ...
              {mobileSheet && <section className="mobile-character-sheet">
                ...
                {mobileSheetTab === "resources" && <div className="mobile-sheet-panel">
                  ...
                  {sharedSpellSlots.length > 0 && <div className="mobile-slot-list"><h3>Ячейки заклинаний</h3>{sharedSpellSlots.map((maximum, circle) => <article key={circle}><span>{circle + 1} круг</span><button onClick={() => setUsedSlots(circle, Math.min(maximum, (character.spellSlotsUsed?.[circle] || 0) + 1), maximum)}>Потратить</button><b>{maximum - (character.spellSlotsUsed?.[circle] || 0)} / {maximum}</b><button onClick={() => setUsedSlots(circle, Math.max(0, (character.spellSlotsUsed?.[circle] || 0) - 1), maximum)}>Вернуть</button></article>)}</div>}
                </div>}
              ...
              <div ref={exportPanelRef} className="export-panel">
                ...
                {sharedSpellSlots.length > 0 && <div className="slot-usage-editor">
                  {sharedSpellSlots.map((maximum, circle) => <label key={circle}><span>{circle + 1} круг</span><input type="number" min="0" max={maximum} value={character.spellSlotsUsed?.[circle] || 0} onChange={event => setUsedSlots(circle, +event.target.value, maximum)} /><small>потрачено из {maximum}</small></label>)}
                </div>}
                {pactMagicSlots.slots > 0 && <div className="slot-usage-editor"><label><span>Ячейки договора · {pactMagicSlots.level} круг</span><input type="number" min="0" max={pactMagicSlots.slots} value={character.pactSlotsUsed || 0} onChange={event => setCharacter(current => ({ ...current, pactSlotsUsed: Math.max(0, Math.min(pactMagicSlots.slots, +event.target.value)) }))} /><small>потрачено из {pactMagicSlots.slots}</small></label></div>}
