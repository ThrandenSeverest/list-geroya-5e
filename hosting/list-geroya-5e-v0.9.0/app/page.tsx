...
type AccountState = { authenticated: true; email: string; displayName: string } | { authenticated: false };
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          {account?.authenticated
            ? <span className="account-state"><span>●</span>{account.displayName} · {cloudState === "saving" ? "сохраняется" : cloudState === "error" ? "ошибка синхронизации" : "в аккаунте"}<a href="/account">Аккаунт</a></span>
            : <span className="account-warning">Без входа персонажи хранятся только в этом браузере и могут потеряться.<a href="/account">Войти и сохранить</a></span>}
        </div>
      </header>
          ...
          {step === 2 && (
            <div className="pointbuy-layout">
              <div className={`pointbuy-status ${pointRemaining === 0 ? "complete" : ""}`} data-incomplete={pointRemaining !== 0}>
                ...
                <small>Стоимость: 8→0 · 9→1 · 10→2 ·  ? "+" : ""}{abilityModifier(finalAbilities[key                 </section>
                ))}
              </div>
          ...
          {step === 10 && (
            <div className="final-workspace">
              ...
              <PdfCharacterSheet
                ...
                attacks={attacks}
                resources={[
                  ...spellRule.slots.map((maximum, circle) => ({ name: `Ячейки ${circle + 1} круга`, current: maximum - (character.spellSlotsUsed?.[circle] || 0), max: maximum })),
                  ...(spellRule.pact ? [{ name: `Ячейки договора (${spellRule.pact.level} круг)`, current: spellRule.pact.slots - (character.pactSlotsUsed || 0), max: spellRule.pact.slots }] : []),
                  ...resources.map(resource => ({ name: resource.name, current: resourceCurrent(exportCharacter, resource), max: resource.max, die: resource.die })),
                ]}
                ...
                spells={[...new Set([...character.spells, ...grantedFeatSpells, ...alwaysPrepared])].map(id => spells.find(spell => spell.id === id)).filter(Boolean).map(spell => ({
                  ...spell!,
                  prepared: selectedPrepared.includes(spell!.id),
