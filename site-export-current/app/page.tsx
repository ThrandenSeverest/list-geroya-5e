...
import { backgrounds, CatalogOption, classes, classSkillRules, races, spells } from "./catalog";
import {
  abilityModifier,
...
import { hitDiceAfterLongRest, hitDieHealing } from "./restRules";
...
export default function Home() {
  ...
  const [experimentalUi, setExperimentalUi] = useState(true);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setExperimentalUi(localStorage.getItem("list-geroya-experimental-ui") !== "disabled");
    } catch {
  ...
  function toggleMobilePreparedSpell(id: string) {
    setCharacter(current => {
      const available = current.spells.filter(spellId => (spells.find(spell => spell.id === spellId)?.level || 0) > 0);
      const initialPrepared = spellRule.mode === "prepared" && !current.mobilePreparedConfigured
        ? available.slice(0, spellRule.prepared || available.length)
      ...
      return { ...current, preparedSpells: next, mobilePreparedConfigured: true };
    });
  }
  ...
  function takeShortRest() {
    setCharacter(current => {
      const nextSpent = { ...(current.resourceSpent || {}) };
      characterResources({ ...current, abilities: finalAbilityScores(current) }).forEach(resource => {
        if (resource.isShortRest) nextSpent[resource.key] = 0;
      });
      return { ...current, resourceSpent: nextSpent, pactSlotsUsed: 0 };
    });
  }
  ...
  function spendHitDie() {
    const remaining = character.level - (character.hitDiceSpent || 0);
    if (remaining <= 0 || (character.currentHitPoints || 0) >= hitPoints) return;
    const die = classRules[character.className]?.hitDie || 8;
    const roll = (crypto.getRandomValues(new Uint32Array(1))[0] % die) + 1;
    const healing = hitDieHealing(roll, abilityModifier(finalAbilities.con));
    setCharacter(current => ({ ...current, currentHitPoints: Math.min(hitPoints, (current.currentHitPoints || 0) + healing), hitDiceSpent: Math.min(current.level, (current.hitDiceSpent || 0) + 1) }));
  ...
  if (view === "characters") {
    ...
    return (
      <main className={`app-shell${experimentalUi ? " modern-design" : ""}`}>
        <header className="topbar">
          <button className="brand" onClick={() => setView("builder")}><span className={`brand-mark${experimentalUi ? " experimental-site-mark" : ""}`}>{experimentalUi ? <img src="/experimental/site-mark.png" alt="" /> : "✦"}</span>Лист Героя <small>5E · 2014</small></button>
          <button className={`experimental-toggle${!experimentalUi ? " active" : ""}`} onClick={() => { const next = !experimentalUi; setExperimentalUi(next); localStorage.setItem("list-geroya-experimental-ui", next ? "enabled" : "disabled"); }} title={experimentalUi ? "Включить старый дизайн" : "Вернуть новый дизайн"}>Дизайн сайта</button>
          ...
          <div className="character-grid">
            {vault.slots.map(slot => {
              ...
              return <article key={slot.id} className={slot.id === vault.activeId ? "active" : ""}>
                <CatalogIcon id={itemClass?.id || itemRace?.id} kind={itemClass ? "class" : "race"} fallback={itemClass?.name || itemRace?.name || "Новый герой"} experimental={experimentalUi} />
  ...
  return (
    <main className={`app-shell${experimentalUi ? " modern-design" : ""}`}>
      <header className="topbar">
        <button className="brand" onClick={() => resetFilters(0)}><span className={`brand-mark${experimentalUi ? " experimental-site-mark" : ""}`}>{experimentalUi ? <img src="/experimental/site-mark.png" alt="" /> : "✦"}</span>Лист Героя <small>5E · 2014</small></button>
        <button className={`experimental-toggle${!experimentalUi ? " active" : ""}`} onClick={() => { const next = !experimentalUi; setExperimentalUi(next); localStorage.setItem("list-geroya-experimental-ui", next ? "enabled" : "disabled"); }} title={experimentalUi ? "Включить старый дизайн" : "Вернуть новый дизайн"}>Дизайн сайта</button>
                ...
                {filtered.map(option => {
                  ...
                  return (
                    <article key={option.id} className={`choice-card ${selected ? "selected" : ""}`}>
                      <CatalogIcon id={option.id} kind={step === 0 ? "race" : step === 1 ? "class" : "background"} fallback={option.name} experimental={experimentalUi} />
                      <div>
            ...
            <div className="final-workspace">
              ...
              {mobileSheet && <section className="mobile-character-sheet">
                ...
                {mobileSheetTab === "combat" && <div className="mobile-sheet-panel">
                  <div className="mobile-hp-card"><label>Текущие хиты<input type="number" min="0" max={hitPoints} value={character.currentHitPoints || 0} onChange={event => setCharacter(current => ({ ...current, currentHitPoints: Math.max(0, Math.min(hitPoints, Number(event.target.value) || 0)) }))} /></label><span>из {hitPoints}</span></div>
                  ...
                  <div className="mobile-rest-row"><button onClick={takeShortRest}>Короткий отдых</button><div><span>к{classRules[character.className]?.hitDie || 8}</span><small>{character.level - (character.hitDiceSpent || 0)} / {character.level}</small></div><button className="hit-die-button" onClick={spendHitDie} disabled={character.level - (character.hitDiceSpent || 0) <= 0 || (character.currentHitPoints || 0) >= hitPoints} aria-label="Потратить кость хитов">+</button><button onClick={takeLongRest}>Длинный отдых</button></div>
                  {lastHitDieRoll !== null && <p className="mobile-roll-result">Восстановлено хитов: <b>{lastHitDieRoll}</b> (кость + модификатор Телосложения)</p>}
                ...
                {mobileSheetTab === "spells" && <div className="mobile-sheet-panel">
                  <div className="mobile-prepared-head"><h3>Подготовленные заклинания</h3><span>{mobilePreparedIds.length} / {spellRule.prepared || 0}</span></div>
                  {spellRule.prepared ? <div className="mobile-spell-list">{selectedLeveled.map(id => { const spell = spells.find(item => item.id === id); return spell ? <button key={id} className={mobilePreparedIds.includes(id) ? "prepared" : ""} onClick={() => toggleMobilePreparedSpell(id)}><span>{mobilePreparedIds.includes(id) ? "●" : "○"}</span><strong>{spell.name}</strong><small>{levelLabel(spell.level)}</small></button> : null; })}</div> : <p>Этот класс не готовит заклинания из изменяемого списка.</p>}
                </div>}
